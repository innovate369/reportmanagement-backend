...
const updateClient = async (req, res) => {
  const { id } = req.params
  const updateById = await Clients.findByIdAndUpdate(
    id,
    req.body,
    { runValidator: true, new: true }
  )
  res.send({ msg: 'Clients data updated succesfully!', data: updateById, status: 200 })
}

module.exports = {
  getAllClients, addClient, deleteClient, clientInvoice, bindProject, getClientById, clientData, updateClient
}
